...
module.exports.boostinfo = function(discordbot, options) {

  const Discord = require("discord.js")

  discordbot.on("guildMemberUpdate", (notBooster, booster) => {

    channel_id = (options && options.channel_id);
    booster_text = (options && options.booster_text);
    notBooster_text = (options && options.notBooster_text);
    ...
    const notBoost = notBooster.premiumSince;
    const boost = booster.premiumSince;

    if(!notBoost && boost) {
      
      if(!booster_text) {

        let booster_embed = new Discord.MessageEmbed()
        .setTitle("New Boost!")
        .setDescription(`${booster.user.tag} has boosted the server!`)
        .setThumbnail(booster.user.displayAvatarURL({dynamic: true}))
        .setColor("GREEN")
        .setTimestamp()

        return discordbot.channels.cache.get(`${channel_id}`).send(booster_embed);
      }

      let booster_text_embed = new Discord.MessageEmbed()
      .setTitle("New Boost!")
      .setDescription(`${booster_text}`)
      .setThumbnail(booster.user.displayAvatarURL({dynamic: true}))
      .setColor("GREEN")
      .setTimestamp()

      return discordbot.channels.cache.get(`${channel_id}`).send(booster_text_embed); 
    }
    ...
    if(notBoost && !boost) {
      ...
      if(!notBooster_text) {
        ...
        let notBooster_embed = new Discord.MessageEmbed()
        .setTitle("Boost Info")
        .setDescription(`${booster.user.tag} has unboosted the server!`)
        .setThumbnail(booster.user.displayAvatarURL({dynamic: true}))
        .setColor("RED")
        .setTimestamp()
        
        return discordbot.channels.cache.get(`${channel_id}`).send(notBooster_embed);
      }

      let notBooster_text_embed = new Discord.MessageEmbed()
      .setTitle("Boost Info")
      .setDescription(`${notBooster_text}`)
      .setThumbnail(booster.user.displayAvatarURL({dynamic: true}))
      .setColor("RED")
      .setTimestamp()
        
      return discordbot.channels.cache.get(`${channel_id}`).send(notBooster_text_embed);
    }
  })
}
...
module.exports.mydevice = function(discordbot, options) {
  ...
  discordbot.on("message", async message => {
    ...
    mydeviceCmd = (options && options.mydeviceCmd) || "mydevice";
    
    if(command === mydeviceCmd) {

      let member = message.author;
      let devices = member.presence.clientStatus || {};
      ...
      let deviceEmbed = new Discord.MessageEmbed()
      .setTitle(`Devices: ${member.tag}`)
      .setThumbnail(member.displayAvatarURL({dynamic: true}))
      .setDescription(`${description()}`)
      .setColor("BLUE")
      .setTimestamp()
  
      message.channel.send(deviceEmbed);
    }
  })
}
    ...
    if(command === creategwCmd) {
      ...
      if(!timer) {
        ...
        if(!timer_error) {
          ...
          .setDescription("Enter time (minutes).")
          .setColor("GREY")
...
module.exports.google = function(discordbot, options) {
  ...
  discordbot.on("message", async message => {
    ...
    googleCmd = (options && options.googleCmd) || "google";

    if(command === googleCmd) {

      text_error = (options && options.text_error);

      let searchText = message.content.split(" ");
      let realSearch = args.slice(0).join("+");

      searchText.shift();
      searchText = searchText.join(" ");

      if(!searchText && !realSearch) {
      ...
      .setDescription(`Successfull Google search!\nSearch: ${searchText}\nResult: [Link](https://www.google.com/search?q=${realSearch}&rlz=1C1CHBF_enRS848RS848&oq=${realSearch}&aqs=chrome..69i57j0i13l2j46i13j0i13i30l6.2288j0j7&sourceid=chrome&ie=UTF-8)`)
      .setThumbnail(discordbot.user.displayAvatarURL({dynamic: true}))
      .setColor("BLUE")
      .setTimestamp()
